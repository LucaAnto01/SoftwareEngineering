...
public class Administrator extends Employee 
{
	...
	private boolean editEmployee()
	{
		...
		try
		{
			...
			if(checkSelectedEmployee(usernameIn))
			{
				...
				for (User currentUser : Main.userList)
				{
					if(currentUser instanceof Employee)
					{
						if(currentUser.getUsername().equals(usernameIn))
						{
	...
	private boolean deleteEmployee()
	{
		...
		try
		{
			...
			if(checkSelectedEmployee(usernameIn))
			{
				...
				for (User currentUser : Main.userList) //Find Worker to delete
					if(currentUser instanceof Employee)
						if(currentUser.getUsername().equals(usernameIn))
							employeeToDelete = (Employee) currentUser;		
				
				if(!employeeToDelete.getUsername().equals("")) //Check if object has value
				{
					Main.userList.remove(employeeToDelete);
					return true;
				}									
			}
		}
	...
	private boolean checkSelectedEmployee(String username)
	{
		...
		for (User currentUser : Main.userList)
		{
			if(currentUser instanceof Employee) //Check if the Worker is a Employee
			{
				if(currentUser.getUsername().equals(username)) //Check username
				{
					exist = true;
					break;
				}								
